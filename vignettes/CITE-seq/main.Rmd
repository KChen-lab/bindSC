---
title: "Integration of scRNA-seq and cytof data"
author: "douj"
...
## Use cells from CITE-seq which have paired gene expresion and protein abundance profiles
Load data from Seurat website 
...
## UMAP visualization of cells in RNA and protein space 

**For convenience, we have prepared the pre-processed data which are ready to us
For RNA, gene expression profiles of 25 proteins could not seperate major immmune cell types 

```{r, message = FALSE, warning=FALSE, results='hide',fig.height=3, fig.width=8}
...
## run bindSC
X: protein abundance level for 25 proteins 
Y: gene expression level for 3000 variable genes
Z: gene expression levle for 25 proteins
...
## Visualization of cells in co-embeddings
The major cells types between RNA and protein could be well aligned
...
## Visualization of cells in co-embeddings for other methods

**For convenience, we have prepared the pre-processed data which are ready to use.** 
Traditional integration methods do not work because only 25 gene profile (RNA) was used for integrtaion 


```{r plot, message = FALSE, warning=FALSE, results='hide',fig.height=6, fig.width=6}
